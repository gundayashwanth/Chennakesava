import React from 'react'
import './Ck.css'
import Navbar from './Nav'
import d1 from './Images/doctor1.png'
import photo from './Images/Ck9.png'
import d2 from './Images/Doctor2.png'
import d3 from './Images/doctor3.png'
import d4 from './Images/doctor4.png'
import { FaInstagram } from "react-icons/fa6";
import { FaFacebook } from "react-icons/fa6";
import { FaYoutube } from "react-icons/fa";
import { FaLocationDot } from "react-icons/fa6";
import { MdEmail } from "react-icons/md";
import { IoCall } from "react-icons/io5";
import Footer from './Footer'
import { Link } from 'react-router-dom';
function Doctors() {
  return (
    <div>
      <Navbar/>
      
      <div className='docimg'>
        <img src={photo}width="100%" height="400px"></img>
      </div>
      <div className='onerow'>
      <div className='doc1'>
      <div className='d1'>
        <img src={d1}width="300px"height="auto"></img>
      </div>

      <div className='doc-container'>
        <h1>DR. Bharathi Raja</h1>
        <h4>MBBS, MS, F.MAS,FIAGES,FALS,COLORECTAL</h4>
        <p>Laproscopic & Lser Surgeon</p>
        <Link to="/apointment" ><button className="appointment-btn">Book a visit</button></Link>
      </div>
      </div>
      <div className='doc2'>
      <div className='d2'>
        <img src={d2}width="300px"height="auto"></img>
      </div>
      <div className='doc-container'>
        <h1>DR. Sashi Kiran</h1>
        <h4>MBBS,MD</h4>
        <p>Paediatric</p>
        <Link to="/apointment" ><button className="appointment-btn">Book a visit</button></Link>
      </div>
      </div>
      </div>
      <div className='secondrow'>
      <div className='doc3'>
      <div className='d3'>
        <img src={d3}width="300px"height="auto"></img>
      </div>
      <div className='doc-container'>
        <h1>DR. Pruthvi</h1>
        <h4>MBBS,MS</h4>
        <p>Urologist</p>
        <Link to="/apointment" ><button className="appointment-btn">Book a visit</button></Link>
      </div>
      </div>
      <div className='doc4'>
      <div className='d4'>
        <img src={d4}width="300px" height="auto"></img>
      </div>
      <div className='doc-container'>
        <h1>DR. Deepika</h1>
        <h4>MBBS,MD, DNB</h4>
        <p>Obstetrician & Gynaechologist</p>
        <Link to="/apointment" ><button className="appointment-btn">Book a visit</button></Link>
      </div>
      </div>
      </div>

      <div className='pk'>
        <div className='bottom'>
        <div className='pkh2'>
            <h1>About</h1>
            <p>CHENNA KESHAVA MULTISPECIALITY HOSPITAL.PVT.LTD <br></br>was opened with a vision of providing quality<br></br> & affordable health care to the residents of Gollarapalya<br></br> & surrounding areas of bangalore. At Chenna Keshava, <br></br>we believe in “Health care for betterment of human race”….</p>
           <div className='icons'>
            <FaInstagram /><FaFacebook /><FaYoutube />
           </div>
        </div>

        <div className='support'>
            <h1>Support</h1>
            <div className="sup-links">
            <Link to="/" >Home</Link><br></br>
            <Link to="/About">About Us</Link><br></br>
            <Link to="/Services">Services</Link><br></br>
            <Link to="/Doctors">Doctors</Link><br></br>
            <Link to="/Gallery">Gallery</Link><br></br>
            <Link to="/Career">Career</Link><br></br>
            <Link to="/Contactus">Contact us</Link><br></br>
      </div>
        </div>

        <div className='location'>
            <h1>Location</h1>
            <FaLocationDot /><p>N0 9/4/9/5, Hosahalli,<br></br>Gollarpalya, Magadi mainroad,<br></br> Herohalli, Bengaluru, <br></br>North Karnataka,- 560091</p>
            <MdEmail /><p>ckmultispeciality hospital@gmail.com</p>
            <IoCall /><p>080-22076871 / 6361242258</p>

        </div>
        </div>

     </div>
     <Footer/>

      
    </div>
  )
}

export default Doctors
